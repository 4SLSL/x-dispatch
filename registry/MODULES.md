...
- Enable/disable lifecycle is host-side: when a module is toggled, X-Dispatch can register/unregister IPC, start/stop runtimes, etc.
  Aujourd’hui, les “hooks lifecycle” sont câblés dans X-Dispatch core (voir `src/main/modulesIpc.ts` + les `src/modules/*/main/lifecycle.ts`).
...
- Les composants de rendu restent dans X-Dispatch core : pour ajouter un nouveau `moduleId` “fonctionnel”, il faut généralement ouvrir un PR X-Dispatch qui met à jour ce mapping (et les éventuels hooks toolbar/carte).
...
- `moduleRendererLoader.ts` — execution not implemented yet.
  Aujourd’hui, le loader renvoie `null`, donc les bundles externes ne sont pas encore exécutés.
